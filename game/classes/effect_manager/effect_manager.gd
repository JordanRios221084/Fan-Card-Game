...
# -- Constants --
const EFFECT_SCENE: PackedScene = preload("res://game/components/effects/ui_effect/scene/ui_effect.tscn") ## Escena del efecto visual.
const ARROW_SCENE: PackedScene = preload("res://game/components/effects/arrow_indicator/scene/arrow_indicator.tscn") ## Escena del indicador de flechas.
...
var background: Background ## Referencia al fondo para cambiar colores.
var foreground: Foreground ## Referencia al primer plano para efectos visuales.
var background_new_color: Color ## Nuevo color de fondo a establecer.
...
func process_effect(card_effects: String, target_player: Player) -> void:
	...
	background.set_background_color(background_new_color, TRANSITION_TIME_SECONDS, COLOR_DIFFERENCE)
...
func set_game_parameters(steps: int, direction: int, bg: Background, fg: Foreground, new_color: Color) -> void:
	...
	background = bg
	foreground = fg
	background_new_color = new_color
...
func _create_arrow_indicator() -> void:
	arrow_indicator = ARROW_SCENE.instantiate()
	add_child(arrow_indicator)
	arrow_indicator.scale = Vector2(3.0, 3.0)
	arrow_indicator.modulate.a = 0.0
...
func _create_ui_effect(value: int, icon: int, target_player: Player, play_burst: bool, on_player: bool) -> void:
	var visual_effect: UIEffect = EFFECT_SCENE.instantiate() as UIEffect
	...
	if play_burst:
		...
		foreground.play_shockwave_effect(visual_effect.get_global_position())
